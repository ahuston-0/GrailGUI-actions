...
class CandlestickChartWidget : public Widget2D {
 ...
 public:
  CandlestickChartWidget(StyledMultiShape2D* m, MultiText* t, float x, float y,
                         float w, float h, const std::string& title,
                         const Style* titleStyle, const Style* barStyle,
                         float min, float max, float maxMultiplier,
                         float relativeSpace, int tickThicknessStart,
                         int tickThicknessEnd, float tickSize, float tickStart,
                         Scale* yAxis)
      : Widget2D(m, t, x, y, w, h),
        ...
        tickThicknessStart(tickThicknessStart),
        tickThicknessEnd(tickThicknessEnd),
        tickSize(tickSize),
        tickStart(tickStart),
        yAxis(yAxis) {}
  ...
  CandlestickChartWidget(StyledMultiShape2D* m, MultiText* t, float x, float y,
                         float w, float h)
      : CandlestickChartWidget(m, t, x, y, w, h, std::string(""), nullptr,
                               nullptr, 0, 100, 1.25, 1.25, 1, 1, 10, x,
                               new LinearScale()) {}

  CandlestickChartWidget(StyledMultiShape2D* m, MultiText* t, float x, float y,
                         float w, float h, const std::vector<float>& b)
      : CandlestickChartWidget(m, t, x, y, w, h, "", nullptr, nullptr, 0, 0,
                               1.25, 1.25, 1, 1, 10, x, new LinearScale()) {

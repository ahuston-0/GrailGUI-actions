...
void Text::init() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * LINE_LENGTH * 6 * 4, (void*)0,
               GL_DYNAMIC_DRAW);
...
void Text::render() {
  ...
  for (int i = 0, j = 0; j < end; i += 24, j++) {
    ...
    vert[i] = (xPos + glyph->bearingX);
    vert[i + 1] = (yPos - glyph->bearingY);
    vert[i + 2] = (glyph->u0);
    vert[i + 3] = (glyph->v1);
    vert[i + 4] = (xPos + glyph->bearingX);
    vert[i + 5] = (yPos - glyph->bearingY + glyph->sizeY);
    vert[i + 6] = (glyph->u0);
    vert[i + 7] = (glyph->v0);
    vert[i + 8] = (xPos + glyph->bearingX + f->maxWidth);
    vert[i + 9] = (yPos - glyph->bearingY + glyph->sizeY);
    vert[i + 10] = (glyph->u1);
    vert[i + 11] = (glyph->v0);

    vert[i + 12] = (xPos + glyph->bearingX);
    vert[i + 13] = (yPos - glyph->bearingY);
    vert[i + 14] = (glyph->u0);
    vert[i + 15] = (glyph->v1);
    vert[i + 16] = (xPos + glyph->bearingX + f->maxWidth);
    vert[i + 17] = (yPos - glyph->bearingY + glyph->sizeY);
    vert[i + 18] = (glyph->u1);
    vert[i + 19] = (glyph->v0);
    vert[i + 20] = (xPos + glyph->bearingX + f->maxWidth);
    vert[i + 21] = (yPos - glyph->bearingY);  // switched
    vert[i + 22] = (glyph->u1);
    vert[i + 23] = (glyph->v1);
  ...
  // cout << "Text color=" <<
  //  style->getFgColor().r << ',' << style->getFgColor().g << ',' <<
  //  style->getFgColor().b << '\n';
...
OLD LOOP, Had size.x instead of maxSize"
  for (int i = 0, j = 0; j < end; i += 24, j++) {
    uint8_t c = text[j];
    const Font::Glyph* glyph = f->getGlyph(c);
    vert[i] =xPos+glyph->bearing.x; vert[i+1] = y-glyph->bearing.y; vert[i+3] =
glyph->v1; vert[i+4]  =xPos+glyph->bearing.x; vert[i+5] =
y-glyph->bearing.y+glyph->size.y; vert[i+7]  = glyph->v0; vert[i+8]
=xPos+glyph->bearing.x+glyph->size.x; vert[i+9] =
y-glyph->bearing.y+glyph->size.y; vert[i+11] = glyph->v0;

    vert[i+12] =xPos+glyph->bearing.x; vert[i+13]=y-glyph->bearing.y; vert[i+15]
= glyph->v1; vert[i+16] =xPos+glyph->bearing.x+glyph->size.x; vert[i+17] =
y-glyph->bearing.y+glyph->size.y; vert[i+19] = glyph->v0; vert[i+20]
=xPos+glyph->bearing.x+glyph->size.x;vert[i+21] = y-glyph->bearing.y; vert[i+23]
= glyph->v1; xPos += glyph->advance;
  }*/
